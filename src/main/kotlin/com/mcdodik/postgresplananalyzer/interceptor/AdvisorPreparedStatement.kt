...
class AdvisorPreparedStatement(
    ...
    override fun setString(parameterIndex: Int, x: String?) {
        params[parameterIndex] = BoundParam(parameterIndex, java.sql.Types.VARCHAR, x?.take(128))
        target.setString(parameterIndex, x)
    }

    override fun setLong(parameterIndex: Int, x: Long) {
        params[parameterIndex] = BoundParam(parameterIndex, java.sql.Types.BIGINT, x.toString())
        target.setLong(parameterIndex, x)
    }

    override fun setInt(parameterIndex: Int, x: Int) {
        params[parameterIndex] = BoundParam(parameterIndex, java.sql.Types.INTEGER, x.toString())
        target.setInt(parameterIndex, x)
    }

    override fun setBoolean(parameterIndex: Int, x: Boolean) {
        params[parameterIndex] = BoundParam(parameterIndex, java.sql.Types.BOOLEAN, x.toString())
        target.setBoolean(parameterIndex, x)
    }

    override fun setTimestamp(parameterIndex: Int, x: java.sql.Timestamp?) {
        params[parameterIndex] = BoundParam(parameterIndex, java.sql.Types.TIMESTAMP, x.toString())
    ...
    override fun execute(): Boolean {
        capture(); return target.execute()
    }

    override fun executeQuery(): ResultSet {
        capture(); return target.executeQuery()
    }

    override fun executeUpdate(): Int {
        capture(); return target.executeUpdate()
    }

    override fun executeBatch(): IntArray {
        capture(); return target.executeBatch()
    }
}